	...
	_gsRequire("plugins.CSSPlugin", ["plugins.TweenPlugin","TweenLite"], function(TweenPlugin, TweenLite) {
		...
		var _NaNExp = /[^\d\-\.]/g,
			...
			_cssDif = function(s1, s2, v, d) { 
				var s = {}, val, p;
				for (p in s2) {
					if (p !== "cssText") if (p !== "length") if (isNaN(p)) if (val !== _transformProp) if (s1[p] != (val = s2[p])) if (typeof(val) === "number" || (typeof(val) === "string" && (val.indexOf(" ") === -1 || val.split(",").length === val.split(" ").length)) ) {
						s[p] = val;
						if (d) {
							d.props.push(p);
						}
			...
			//parses the transform values for an element, returning an object with x, y, scaleX, scaleY, rotation, and skewX properties.
		...
		p._onInitTween = function(target, value, tween) {
			...
			if (typeof(value) === "string") { 
				...
				start = _getStyles(target, cs);
				target.className = (value.className.indexOf("=") === -1) ? value.className : (!value.className.indexOf("+")) ? target.className + " " + value.className.substr(2) : target.className.split(value.className.substr(2)).join("");
		...
		p._parseVars = function(vars, t, cs, map) {
			...
			for (p in vars) {
				...
				if (p === "transform" || p === _transformProp) {
					...
					this._parseVars({paddingTop:v, paddingRight:v, paddingBottom:v, paddingLeft:v}, t, cs, map);
					continue;
				} else if (p === "margin") {
					this._parseVars({marginTop:v, marginRight:v, marginBottom:v, marginLeft:v}, t, cs, map);
					continue;
				} else if (p === "autoRound") {
					continue;
				}
				...
				//r:	*	round						[boolean]
				//type:		0=normal, 1=color, 2=rgba, 3=unsupported opacity (ie), -1=non-tweening prop	[number]
				...
				//if it's an autoAlpha, add a new PropTween for "visibility".
				if (p === "opacity") if (vars.autoAlpha != null) {
					this._firstPT = {_next:pt, t:s, p:"visibility", f:false, n:"css_visibility", r:false, type:-1, b:(Number(beg) !== 0) ? "visible" : "hidden", i:"visible", e:(Number(v) === 0) ? "hidden" : "visible"};
					pt._prev = this._firstPT;
				...
				if (p === "color" || p === "fill" || p.indexOf("Color") !== -1 || (typeof(v) === "string" && !v.indexOf("rgb("))) { //Opera uses background: to define color sometimes in addition to backgroundColor:
					...
					if (beg === "" || beg === "auto") {
						...
						start = parseFloat(beg.replace(_NaNExp, ""));
					}
					...
					if ((start || start === 0) && (v || v === 0) && start !== v) { //faster than isNaN()
						pt.s = start;
						pt.c = (rel ? v : v - start);
						pt.sfx = esfx;
						if (p === "opacity") {
						...
						} else if (p === "zIndex" || (esfx === "px" && vars.autoRound !== false)) { //always round zIndex, and as long as autoRound isn't false, round pixel values (that improves performance in browsers typically)
							pt.r = true;
						}
					} else {
						pt.type = -1;
						pt.i = (p !== "visibility" && p !== _transformProp + "Origin") ? pt.b : pt.e; 
						pt.s = pt.c = 0;
					}
		...
		p._parseTransform = function(t, v, cs, map) {
			...
			if (typeof(v) === "object") { //for values like scaleX, scaleY, rotation, x, y, skewX, and skewY or transform:{...} (object)
				...
				if (v.skewY != null) {
					skewY = (typeof(v.skewY) === "number") ? v.skewY * _DEG2RAD : _parseAngle(v.skewY, 0);
					m2.skewX += skewY;
					m2.rotation += skewY;
				}
				...
				if ((orig = v.transformOrigin) != null) {
					...
					//for older versions of IE (6-8), we need to manually calculate things inside the setRatio() function. We record origin x and y (ox and oy) and whether or not the values are percentages (oxp and oyp). 
						...
						var ox = (orig.indexOf("left") !== -1) ? "0%" : (orig.indexOf("right") !== -1) ? "100%" : orig.split(" ")[0],
							oy = (orig.indexOf("top") !== -1) ? "0%" : (orig.indexOf("bottom") !== -1) ? "100%" : orig.split(" ")[1];
						if (oy == null) {
							oy = "50%"
						}
						m1.oxp = (ox.indexOf("%") !== -1);
		...
		p.setRatio = function(v) {
			...
			if (v === 1 && (this._tween._time === this._tween._duration || this._tween._time === 0)) {
				while (pt) {
					pt.t[pt.p] = pt.e;
					if (pt.type === 3) if (pt.s + pt.c === 1) { //for older versions of IE that need to use a filter to apply opacity, we should remove the filter if opacity hits 1 in order to improve performance.
				...
				while (pt) {
					...
					if (!pt.type) {
						...
						pt.t[pt.p] = pt.i;
					} else if (pt.t.filter.indexOf("opacity=") === -1) { //only used if browser doesn't support the standard opacity style property (IE 7 and 8)
						pt.t.filter += " alpha(opacity=" + (val * 100) + ")";
					} else {
						pt.t.filter = pt.t.filter.replace(_opacityExp, "opacity=" + (val * 100));
					}
					pt = pt._next;
				}
			...
			//if the tween is reversed all the way back to the beginning, we need to restore the original values which may have different units (like % instead of px or em or whatever).
				...
				while (pt) {
					pt.t[pt.p] = pt.b;
					if (pt.type === 3) if (pt.s === 1) { //for older versions of IE that need to use a filter to apply opacity, we should remove the filter if opacity hits 1 in order to improve performance. 
						this._style.removeAttribute("filter");
					}
					pt = pt._next;
				}
			...
			//apply transform tweens, like x, y, scaleX, scaleY, rotation, skewX, or skewY. We do these after looping through all the PropTweens because those are where the changes are made to scaleX/scaleY/rotation/skewX/skewY/x/y.
			if (this._transform) {
				pt = this._transform;
				...
				if (_transformProp && !pt.rotation) {
					//console.log("hit "+((pt.x || pt.y) ? "translate(" + pt.x + "px," + pt.y + "px) " : "") + (pt.rotation ? "rotate(" + pt.rotation + "rad) " : "") + (pt.skewX ? "skewX(" + pt.skewX + "rad) " : "") + ((pt.scaleX !== 1 || pt.scaleY !== 1) ? "scale(" + pt.scaleX + "," + (Math.cos(pt.skewX) * pt.scaleY) + ")" : ""));
					this._style[_transformProp] = ((pt.x || pt.y) ? "translate(" + pt.x + "px," + pt.y + "px) " : "") + (pt.skewX ? "skewX(" + pt.skewX + "rad) " : "") + ((pt.scaleX !== 1 || pt.scaleY !== 1) ? "scale(" + pt.scaleX + "," + (Math.cos(pt.skewX) * pt.scaleY) + ")" : "");
				} else {
					//console.log("bad "+_transformProp);
			...
			if (this._classData) {
				...
				if (v === 1 && this._tween._time === this._tween._duration) {
